...
export class LaunchDisplayComponent implements OnInit {
  ...
  constructor( private sofs: SmartOnFhirService, private topicService:TopicService ) {
    this.smartApps = environment.patientSmartApps;
  }

  ngOnInit() {
    this.topicService.getLaunchInfo( this.launch ).subscribe( next => {
      console.log( next );
  ...
  launchApp(app: any) {
    ...
    a.addEventListener( 'message', this.receiveMessage.bind(this), false);
    console.log(a);
  }
