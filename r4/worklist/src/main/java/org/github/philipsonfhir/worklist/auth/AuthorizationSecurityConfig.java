package org.github.philipsonfhir.worklist.auth;
...
//            .and()
                ...
                .accessTokenValiditySeconds(3600)
                .redirectUris("http://localhost:8080/test","http://localhost:4401/")
            ...
            .and()
                .withClient("eventviewer")
                        ...
                        .scopes("fhircast")
                        .accessTokenValiditySeconds(3600)
                        .redirectUris("http://localhost:4402")
                    ...
                    scope -> {
                        switch( scope ){
                            case "fhircast":
                                ...
                                if ( launch !=null ) {
                                    FhirCastTopic fhirCastTopic = topicService.getTopicFromLaunch(launch);
                                    LaunchContext launchContext = fhirCastTopic.getFhirCastTopicStudyOrPatient(launch);
                                    additionalInfo.put("patient", launchContext.getPatient().getId());
                                    additionalInfo.put("encounter", launchContext.getEncounter().getId());

                                    if ( fhirCastTopic !=null ) {
                                        additionalInfo.put("cast-hub", fhirCastTopic.getTopic());
                                        additionalInfo.put("hub.topic", fhirCastTopic.getTopic());
                                    }
                                }
                                break;
                        }
