...
// http://build.fhir.org/ig/HL7/FHIRPath/branches/master/N1/index.html#date-time-arithmetic

public class MyFHIRPathEngine extends FHIRPathEngine {
...
//    }
     ...
     */
    public MyFHIRPathEngine(IWorkerContext worker) {
        super( worker );
        this.worker = worker;
...
//    }
    ...
    private boolean checkParameters(FHIRLexer lexer, SourceLocation location, ExpressionNode exp, FunctionDetails details) throws FHIRLexerException {
        switch (exp.getFunction()) {
            ...
            case ToQuantity: return checkParamCount(lexer, location, exp, 1);
    ...
    private List<Base> execute(ExecutionContext context, List<Base> focusOrg, ExpressionNode exp, boolean atEntry) throws FHIRException {
//    System.out.println("Evaluate {'"+exp.toString()+"'} on "+focus.toString());
        List<Base> focus = new ArrayList<>();
        for ( Base base: focusOrg ){
            if ( base instanceof BaseList ){
                focus.addAll( ((BaseList)base).getList() );
            } else {
                focus.add( base );
            }
        }
...
//    System.out.println("Result of {'"+exp.toString()+"'}: "+work.toString());
    ...
    private List<Base> opPlus(List<Base> left, List<Base> right) throws PathEngineException {
        ...
        if (!left.get(0).isPrimitive() && left.get(0) instanceof Quantity ){
            left.forEach( leftBase -> {
                try {
                    Quantity rightQuantity = (Quantity) right.get(0);
                    Quantity leftQuantity = (Quantity) leftBase;
                    Decimal leftDecimal = new Decimal( leftQuantity.getValue().toString() );
                    Decimal rightDecimal = new Decimal( rightQuantity.getValue().toString() );

                    // convert to left unit.
                    rightDecimal = this.worker.getUcumService().convert( rightDecimal, rightQuantity.getCode(), leftQuantity.getCode() );
                    Decimal resultDecimal = leftDecimal.subtract(rightDecimal);
                    Quantity resultQuantity = leftQuantity.copy();
                    resultQuantity.setValue( resultDecimal.asInteger() );
                    result.add( resultQuantity );
                } catch (UcumException e) {
                    throw new PathEngineException(String.format("Error performing -: ucum error(%s)", e.getMessage()));
                }
            });
            return result;
        }
...
//            throw new PathEngineException(String.format("Error performing -: right operand has the wrong type (%s)", right.get(0).fhirType()));
    ...
    private List<Base> opDivideBy(List<Base> left, List<Base> right) throws PathEngineException {
        ...
        if (l.hasType("integer", "decimal", "unsignedInt", "positiveInt") && r.hasType("integer", "decimal", "unsignedInt", "positiveInt")) {
            ...
            Quantity ql = (Quantity) l;
            Quantity qr = (Quantity) r;
            double dl = ((Quantity)l).getValue().doubleValue();
            double dr = ((Quantity)r).getValue().doubleValue();

            double dRes = dl/dr;
            String resUnit = ql.getCode()+"/"+qr.getCode();

            result.add( new Quantity()
                    .setValue( dRes )
                    .setCode( resUnit )
                    .setUnit( resUnit )
                    .setSystem( ql.getSystem() )
            );
        } else if (l instanceof Quantity ) {
            Quantity ql = (Quantity) l;

            double dl = ((Quantity)l).getValue().doubleValue();
            double dr;
            if ( r instanceof IntegerType ){
                dr = ((IntegerType)r).getValue();
            } else if ( l instanceof DecimalType ){
                dr = ((DecimalType)r).getValue().doubleValue();
            } else{
                throw new PathEngineException(String.format("Error performing /: left and right operand have incompatible or illegal types (%s, %s)", left.get(0).fhirType(), right.get(0).fhirType()));
            }

            double dRes = dl/dr;
            String resUnit = ql.getCode();
    ...
    private List<Base> funcToQuantity(ExecutionContext context, List<Base> focus, ExpressionNode exp) {
        ...
        if (focus.size() == 1) {
            ...
            if ( exp.getParameters().size()==1 ){
                targetUnit = ((StringType) exp.getParameters().get(0).getConstant()).getValue();
            }

            Quantity q = null;
            if (focus.get(0) instanceof Quantity) {
                q = (Quantity) focus.get(0);
                if ( q!=null ){
                    Decimal dec = null;
                    try {
                        dec = new Decimal(q.getValue().toString());
                        Decimal convDec = this.worker.getUcumService().convert(dec, q.getCode(), targetUnit );
                        q.setValue( Double.parseDouble( convDec.asScientific()));
                        q.setCode( targetUnit).setUnit( targetUnit );
                    } catch (UcumException e) {
                       throw new PathEngineException("Something failed in conversion "+e.getMessage());
                    }
                }
            } else if (focus.get(0) instanceof StringType) {
                q = parseQuantityString(focus.get(0).primitiveValue());
            } else if (focus.get(0) instanceof IntegerType) {
                q = new Quantity().setValue(new BigDecimal(focus.get(0).primitiveValue())).setSystem("http://unitsofmeasure.org").setCode(targetUnit);
            } else if (focus.get(0) instanceof DecimalType) {
                q= new Quantity().setValue(new BigDecimal(focus.get(0).primitiveValue())).setSystem("http://unitsofmeasure.org").setCode(targetUnit);
            }
            result.add( q );
...
//  return result;
    ...
    private List<Base> funcSqrt(ExecutionContext context, List<Base> focus, ExpressionNode exp){
        List<Base> result = new ArrayList<Base>();
        if (focus.size() == 1) {
            if ( focus.get(0) instanceof  DecimalType ) {
                DecimalType dec = (DecimalType) focus.get(0);
                DecimalType sqrt = new DecimalType();
                sqrt.setValue(Math.sqrt(dec.getValue().doubleValue()));
                result.add( sqrt );
            } else if ( focus.get(0) instanceof  IntegerType ) {
                IntegerType dec = (IntegerType) focus.get(0);
                DecimalType sqrt = new DecimalType();
                sqrt.setValue(Math.sqrt(dec.getValue().doubleValue()));
                result.add( sqrt );
            } else if ( focus.get(0) instanceof  Quantity ) {
                Quantity q = (Quantity) focus.get(0);
                Quantity sqrt = q.copy();
                sqrt.setValue(Math.sqrt(q.getValue().doubleValue()));
                result.add( sqrt );
            } else {
                throw new PathEngineException("sqrt cannot be applied to non DecimalTypes");
            }
        }
        return result;
    }

    private List<Base> funcIndexOf(ExecutionContext context, List<Base> focus, ExpressionNode exp){
        List<Base> result = new ArrayList<Base>();
        if (focus.size() == 1) {
            StringType str = (StringType) focus.get(0);
            String s = convertToString(focus.get(0));
            if (!Utilities.noString(s))
                result.add(new DecimalType(s.indexOf( str.getValue() )));
        }
        return result;
    }
